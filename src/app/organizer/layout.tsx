...
export default async function OrganizerLayout({ children }: { children: React.ReactNode }) {
  ...
  // The organizer console nav only appears once the user has created at least
  // one event (admins always see it) — before that the pages render bare, with
  // just the "Create event" call to action.
  const eventCount = await prisma.event.count({ where: { organizerId: user.id } });
  const showNav = eventCount > 0 || user.role === 'admin';
