...
- DondeGO берет **5% комиссии** — только с билетов событий, созданных на
  платформе (нативные DondeGO-события). У агрегированных (внешних) событий
  билеты покупаются на сайте источника, комиссии нет.
- Организатор получает 95%.
- Комиссия считается на сервере в `src/lib/finance/commission.ts`
  (`PLATFORM_COMMISSION_PCT`). Внимание: константа в коде сейчас `10` — её и
  `tests/unit/commission.test.ts` нужно обновить до `5`, чтобы код совпал с
  этой политикой (это меняет ставку на проде dondego.cl).
